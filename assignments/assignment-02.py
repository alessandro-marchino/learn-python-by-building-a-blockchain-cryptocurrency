# 1) Create a list of names and use a for loop to output the length of each
#    name (len() ).
...
# 2) Add an if  check inside the loop to only output names longer than 5
#    characters.
for name in names:
    if len(name) > 5:
        print(f'Name {name} is {len(name)} long')
...
# 3) Add another if  check to see whether a name includes a "n" or "N"
#    character.
for name in names:
